...
function agregarNumeros(arreglo) {
  // "arreglo" debe ser un arreglo de enteros (int/integers)
...
function promedioResultadosTest(resultadosTest) {
  ...
  return Math.ceil(resultado/resultadosTest.length)

}


function numeroMasGrande(arreglo) {
  // "arreglo" debe ser una matriz de enteros (int/integers)
  // Devuelve el número más grande
  // Tu código:
  var resultado=0
  for (var i=0; i<arreglo.length;i++){
    for (var j=0; j<arreglo.length;j++){
      if((arreglo[j]>arreglo[i])&&(arreglo[j]>resultado)){
        resultado=arreglo[j]
      }
    }
  }
...
function breakStatement(numero) {
  ...
  for (var i=1;i<=10;i++){
     if (arreglo.length===0){
       ...
       sumax=arreglo[arreglo.length-1]+2
       arreglo.push(sumax)
       if(sumax===i){
